...
export default function EditArticlePage() {
  ...
  useEffect(() => {
    if (!articleId || isNaN(articleId)) { router.push('/admin'); return; }
    Promise.all([api.getCategories(), api.adminGetArticle(articleId)])
      .then(([cats, article]) => {
        setCategories(cats);
  ...
  const handleSave = async (pub?: boolean) => {
    ...
    setSaving(true);
    try {
      await api.adminUpdateArticle(articleId, {
  ...
  if (loading) return (
    <div className="flex items-center justify-center h-screen">
      <p className="text-sm text-[#9b9b9b]">Loading article...</p>
    </div>
  );
    ...
    <div className="min-h-screen">
      ...
      <div className="border-b border-[#e5e5e3] px-8 py-4 flex items-center justify-between bg-[#fafaf8] sticky top-0 z-10">
        <div className="flex items-center gap-4">
          ...
          <h1 className="font-serif text-lg font-bold text-[#0a0a0a]">Edit Article</h1>
          <span className="text-xs text-[#9b9b9b] flex items-center gap-1">
            <Clock size={11} /> {readingTime} min read
          </span>
        </div>
        <div className="flex items-center gap-2">
          <span className={`text-xs px-2 py-1 ${published ? 'bg-[#0a0a0a] text-white' : 'bg-[#e5e5e3] text-[#6b6b6b]'}`}>
          ...
          {published && (
            <button onClick={() => handleSave(false)} disabled={saving}
              className="flex items-center gap-2 border border-[#e5e5e3] text-[#6b6b6b] px-4 py-2 text-sm font-medium hover:bg-[#f0f0ee] disabled:opacity-50 transition-colors">
              Unpublish
            </button>
          )}
      ...
      <div className="max-w-4xl mx-auto px-8 py-8">
        ...
        <div className="flex items-center gap-2 mb-4">
          <span className="text-xs text-[#9b9b9b]">URL: /article/</span>
          ...
          <button onClick={() => { setSlug(buildSlug(title)); setSlugEdited(false); }}
            className="text-[#9b9b9b] hover:text-[#0a0a0a]" title="Regenerate slug">
            <RefreshCw size={12} />
          </button>
        </div>

        {/* Excerpt */}
        <textarea placeholder="Short excerpt..."
        ...
        <div className="mb-6">
          ...
          <div className="flex items-center gap-3">
            <input type="text" placeholder="Paste URL or upload"
          ...
          {coverImage && <img src={coverImage} alt="Cover" className="mt-3 max-h-40 object-cover border border-[#e5e5e3]" />}
        </div>

        {/* Content */}
        <div className="mb-6">
          <label className="block text-xs uppercase tracking-widest text-[#9b9b9b] mb-2 font-medium">Content</label>
          {content !== null && <RichEditor key={articleId} content={content} onChange={setContent} />}
        </div>
            ...
            <div className="px-4 pb-4 space-y-4 border-t border-[#e5e5e3] pt-4">
              <div>
                <label className="block text-xs uppercase tracking-widest text-[#9b9b9b] mb-1.5 font-medium">
                  Meta Description <span className="normal-case">(~150 chars)</span>
                </label>
                <textarea value={metaDescription} onChange={e => setMetaDescription(e.target.value)}
                  placeholder="For search engines..." rows={2} maxLength={160}
